...
async function aceptarPrivacidad(page) {
  ...
  try {
    ...
    // Si falla algo en la aceptación, evitamos que el bot crashee
  }
}
/**
...
async function buscarRutaExacta(inputUsuario) {
  ...
  if (coincidencias.length === 0) {
    throw new InputError(
      `No encontré ninguna ruta que coincida con "${inputUsuario}".`,
      {
        visible: true,
  ...
  if (coincidencias.length > 1) {
    ...
    const coincidenciaExacta = coincidencias.find(
      (r) => limpiarTextoBusqueda(r) === inputLimpio,
    );
    ...
    throw new InputError(
      `Hay varias rutas parecidas a "${inputUsuario}". ¿Cuál de estas buscas?`,
      {
        visible: true,
...
async function watchRoute(ruta) {
  return taskQueue.add(async () => {
    ...
    try {
      ...
      const botonMenuVerRutas = await page.locator(
        'button:has-text("Ver una línea")',
      );
      ...
      if (!existe) {
        logger.warn(
          `Cuando falla una ruta: Ruta "${nombreRutaExacto}" no encontrada en el DOM`,
        );
      ...
      logger.info(
        `Cuando una ruta se encuentra: Preparando captura para "${nombreRutaExacto}"`,
      );
      ...
      logger.info(
        `Fin del scraper: captura guardada temporalmente en ${filePath}`,
      );
      return filePath;
    } catch (error) {
      if (error instanceof InputError) throw error;
      throw new ScraperError(
        `Error al capturar la ruta "${nombreRutaExacto}"`,
        { cause: error },
      );
    }
  });
